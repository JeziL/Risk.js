...
function Editor(containerID, options) {
  ...
  this.addLoadCounter();
  this.addToolsButton('playnode_button', 'Start', 'imgs/icon-play.png', this.onPlayButton.bind(this), '.tools-right');
  // this.addToolsButton(
  //     "playstepnode_button",
  //     "Step",
...
Editor.prototype.addToolsButton = function (id, name, iconURL, callback, container) {
  ...
  var button = this.createButton(name, iconURL, callback);
  button.id = id;
  this.root.querySelector(container).appendChild(button);
};

Editor.prototype.createButton = function (name, iconURL, callback) {
...
Editor.prototype.onLiveButton = function () {
  var isLiveMode = !this.graphcanvas.live_mode;
  this.graphcanvas.switchLiveMode(true);
  this.graphcanvas.draw();
  var button = this.root.querySelector('#livemode_button');
  button.innerHTML = !isLiveMode ? '<img src="imgs/icon-record.png"/> Live' : '<img src="imgs/icon-gear.png"/> Edit';
};
...
Editor.prototype.addMiniWindow = function (w, h) {
  ...
  graphcanvas.onRenderBackground = function (_canvas, ctx) {
    ctx.strokeStyle = '#567';
